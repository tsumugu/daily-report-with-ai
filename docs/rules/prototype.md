...
**Storybook** を使用してプロトタイプを作成する。

- 既存のStorybook環境を活用
- 画面全体のフローをストーリーとして表現
- インタラクションは `play` 関数や `args` で制御
...
- **Storybookストーリー**: `apps/web/src/stories/prototypes/{feature_name}/`
...
各画面（ページ）ごとにストーリーを作成する：

```typescript
// apps/web/src/stories/prototypes/daily-report-input/daily-report-input.stories.ts
export default {
  title: "Prototypes/DailyReportInput",
  component: DailyReportInputPage,
} as Meta;
...
1. **複数のストーリーで表現**: 各画面を別ストーリーとして作成し、遷移先をコメントで明記
2. **インタラクティブな遷移**: `play` 関数を使用してクリックで次の状態に遷移
3. **状態管理**: コンポーネント内で状態を管理し、ボタンクリックで状態遷移
