...
- ページヘッダー：戻るボタン、ページタイトル（例: 「よかったことのフォローアップ」）
- エピソード/アクション一覧セクション：
  - セクションヘッダー：タイトル（例: 「再現エピソード」）、ステータス表示（例: 「進行中 2/3件」）
  - エピソード/アクションカード一覧：登録済みエピソード/アクションの一覧表示
  - エピソード/アクション追加ボタン：新規登録用ボタン（破線ボーダー）

### 2.2 エピソード/アクション追加フォームモーダル

- モーダルオーバーレイ：半透明背景
- モーダルヘッダー：タイトル（例: 「エピソードを追加」）、閉じるボタン
- エピソード/アクション入力フォーム：日付入力（必須）、メモ入力（最大500文字、文字数カウンター付き）
- モーダルフッター：キャンセル、追加ボタン
...
- ページ全体：最大幅800px、中央配置
- ページヘッダー：固定ヘッダー（sticky）、戻るボタンとタイトルを横並び
- エピソード/アクション一覧：
  - セクションヘッダー：タイトルとステータスを横並び
  - 縦方向にリスト表示（日付の降順、新しい順）
  - 各エピソード/アクションはカード形式で表示（背景色: `--color-bg-secondary`）
  - 削除ボタンは右側に配置
  - 追加ボタンは一覧の下に配置（破線ボーダーで視覚的に区別）
  - **メモの長文表示**: メモが長文の場合は、3行まで表示し、それ以上は「もっと見る」ボタンで展開（Phase 3以降で検討）

### 4.2 エピソード/アクション追加フォームモーダル
...
- 「エピソードを追加」/「アクションを追加」ボタンクリック時：エピソード/アクション追加フォームモーダルを表示
- フォーム入力後「追加」ボタンクリック時：エピソード/アクションを一覧に追加、モーダルを閉じる、ステータスを自動更新
- キャンセルボタン/×ボタン/オーバーレイクリック/Escapeキーでモーダルを閉じる
...
- 「フォローアップ」ボタンクリック時：フォローアップ画面にページ遷移
- フォローアップ画面での動作はフォロー項目一覧画面と同様
...
- [フォローアップ画面HTML](./mockups/followup-page.html)
- [フォローアップ画面CSS](./mockups/followup-page.css)

**主な特徴**:

- **ページ遷移によるフォローアップ画面**
  - ページヘッダー（戻るボタン、タイトル）- sticky配置
  - セクションヘッダー（タイトル、ステータス表示）
  - エピソードカードの一覧表示（背景色: `--color-bg-secondary`）
  - ステータスと登録数の表示（例: 「進行中 2/3件」）
  - エピソード追加ボタン（破線ボーダー、ホバー時は青のボーダーと薄い青の背景）
- **エピソード追加フォームモーダル（オーバーレイ）**
  - モーダルオーバーレイ（半透明背景、z-index: 2000）
  - エピソード入力フォーム（日付入力必須、メモ入力最大500文字、文字数カウンター）
  - Escapeキーで閉じる機能
  - オーバーレイクリックで閉じる機能
  - ×ボタン、キャンセルボタンで閉じる機能
- **アニメーション**
  - エピソード追加時のフェードイン（200ms）
  - モーダル表示時のフェードイン（200ms）・スライドアップ（300ms）
...
### 11.3 エピソード/アクション追加フォームモーダル

- モーダル表示時：フェードインアニメーション（200ms）、スライドアップアニメーション（300ms）
- モーダル非表示時：フェードアウトアニメーション（200ms）
...
- エピソード/アクションカードは全幅表示
- 削除ボタンは右端に固定
- セクションヘッダー：縦方向に配置（タイトルとステータス）
- エピソード/アクション追加フォームモーダル：幅95%

### 12.2 タブレット・PC（768px〜）

- ページ全体：最大幅800pxで中央配置
- エピソード/アクションカード：全幅表示
- エピソード/アクション追加フォームモーダル：最大幅500px
...
- モーダル開閉時のフォーカス管理：モーダル開時は最初の入力フィールドにフォーカス、閉時は開いたボタンにフォーカスを戻す
...
- **ラベル**: 「フォローアップ」
- **動作**: クリックでフォローアップ入力画面にページ遷移（tech_spec.mdと整合性を保つ）
